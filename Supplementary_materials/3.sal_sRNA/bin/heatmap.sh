...
cut -f1 ${DIR}/nhmmer/trainingSeqs.gff ${DIR}/nhmmer/startingSeqs.gff | sort | uniq | xargs -ifoo grep foo pseudomonas_summary.txt | tr -d '>'  > ${DIR}/data/Ents.txt
...
getSeqs(){
    ...
    for i in `cut -f2 ${DIR}/data/Ents.txt | sort | uniq -d -c | awk '{if($1>2){print $2}}'`; do
	grep -E "\s+$i$" ${DIR}/data/Ents.txt | cut -f1 > ${DIR}/data/heatmap/$i.genus
    done
}
